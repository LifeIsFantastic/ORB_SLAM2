...
template <class T>
void DeviceArray<T>::uploadAsync(const T *hptr, cudaStream_t &stream, uint start, uint count)
{
  if(count == 0)
  {
    count = static_cast<uint>(size_);
  ...
  checkCudaErrors(cudaMemcpyAsync(reinterpret_cast<void *>(dptr_[current_read_] + start), reinterpret_cast<const void *>(hptr + start), count * sizeof(T), cudaMemcpyHostToDevice, stream));
...
template <class T>
void DeviceArray2D<T>::uploadAsync(const T *hptr, cudaStream_t &stream)
{
  // width in bytes
  size_t width_bytes = width_ * sizeof(T);

  checkCudaErrors(cudaMemcpy2DAsync(reinterpret_cast<void *>(dptr_[current_read_]), pitch_, reinterpret_cast<const void *>(hptr), width_bytes, width_bytes, height_, cudaMemcpyHostToDevice, stream));
}
} // namespace
