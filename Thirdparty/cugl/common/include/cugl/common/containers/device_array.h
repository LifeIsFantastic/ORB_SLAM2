...
#include <cugl/common/helper_gl.h>
#include <cugl/common/helper_cuda.h>
#include <cugl/common/helper_cuda_gl.h>

#include <cugl/common/containers/device_array_handle.h>
...
class DeviceArray
{
 public:
  ...
  void uploadAsync(const T *hptr, cudaStream_t &stream, uint start = 0, uint count = 0);
...
class DeviceArray2D
{
 public:
  ...
  DeviceArrayHandle2D<T> getHandle(uint offset_h)
  {
    T *row_entry = reinterpret_cast<T *>(reinterpret_cast<char *>(this->getDevicePtr()) + offset_h * this->getPitch());

    DeviceArrayHandle2D<T> handle(row_entry, this->getPitch());
    return handle;
  }
  ...
  DeviceArrayHandle2D<T> getWriteHandle()
  {
    DeviceArrayHandle2D<T> handle(this->getDeviceWritePtr(), this->getPitch());
    return handle;
  }

  DeviceArrayHandle2D<T> getWriteHandle(uint offset_h)
  {
    T *row_entry = reinterpret_cast<T *>(reinterpret_cast<char *>(this->getDeviceWritePtr()) + offset_h * this->getPitch());
  ...
  void uploadAsync(const T *hptr, cudaStream_t &stream);
...
#include <cugl/common/containers/impl/device_array.hpp>
