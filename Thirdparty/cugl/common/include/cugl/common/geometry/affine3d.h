...
#include <cugl/common/helper_math.h>
...
class Affine3d
{
 public:
  ...
  __CUGL_HOST_DEVICE__ Affine3d(const float4 &rot_quat, const float4 &trans_vec)
  {
    setRotFromQuat(rot_quat);

    setTransFromVec(trans_vec);
  }
  ...
  __CUGL_HOST_DEVICE__ void setRotFromQuat(const float4 &quat)
  {
    float q00 = quat.x * quat.x;
    float q01 = quat.x * quat.y;
    float q02 = quat.x * quat.z;
    float q03 = quat.x * quat.w;

    float q11 = quat.y * quat.y;
    float q12 = quat.y * quat.z;
    float q13 = quat.y * quat.w;

    float q22 = quat.z * quat.z;
    float q23 = quat.z * quat.w;

    float q33 = quat.w * quat.w;

    float s = 1.f / (q00 + q11 + q22 + q33);

    data[0] = s * (q00 + q11 - q22 - q33);
    data[1] = s * 2 * (q12 + q03);
    data[2] = s * 2 * (q13 - q02);
    data[3] = 0.f;

    data[4] = s * 2 * (q12 - q03);
    data[5] = s * (q00 - q11 + q22 - q33);
    data[6] = s * 2 * (q23 + q01);
    data[7] = 0.f;

    data[8] = s * 2 * (q13 + q02);
    data[9] = s * 2 * (q23 - q01);
    data[10] = s * (q00 - q11 - q22 + q33);
    data[11] = 0.f;
  }

  __CUGL_HOST_DEVICE__ void setTransFromVec(const float4 &vec)
  {
    data[12] = vec.x;
    data[13] = vec.y;
    data[14] = vec.z;
